...
namespace Maliyet_Takip.Forms.StokForms
{
    public partial class StokEditForm : BaseEditForm
    {
        ...
        public StokEditForm(int id, bool ac)
        {
            _ac = ac;
            _id = id;
            InitializeComponent();
        ...
        private void StokGetir(int id)
        {
            ...
            try
            {
                komut = new SqlCommand("Select * from Hammaddeler where Id = '" + _id + "'", baglan.bgl());
                ...
                {
                    komut = new SqlCommand("Select * from HammaddeGruplar where Id = '" + txtStokGrubu.Id + "'", baglan.bgl());
                    oku = komut.ExecuteReader();
                    while (oku.Read())
                    {
        ...
        protected override bool Kaydet()
        {
            if (txtStokAdi.Text != "")
            {
                if (txtBarkodu.Text != "")
                {
                    if (txtAltLimit.Text != "")
                    {
                        var dr = mesajlar.EvetSeciliEvetHayir(txtStokAdi.Text + " hammadde işlemini onaylıyor musunuz?", "Uyarı");
                        if (dr == System.Windows.Forms.DialogResult.Yes)
                        {
                            try
                            {
                                ...
                                {
                                    int sayac = 0;
                                    komut = new SqlCommand("Select * from Hammaddeler where StokAdi = '" + txtStokAdi.Text + "'", baglan.bgl());
                                    oku = komut.ExecuteReader();
                                    while (oku.Read())
                                    {
                                        sayac++;
                                    ...
                                    if (sayac > 0)
                                    {
                                        mesajlar.Hata("Stok Adıyla daha önce bir hammadde oluşturulmuş farklı bir hammadde adı giriniz");
                                        txtStokAdi.Focus();
                                        return false;
                                    }
                                }
                                ...
                                if (!_ac)
                                {
                                    komut = new SqlCommand("insert into Hammaddeler (Kod, StokAdi, Barkodu, Birimi, AlisKDV, SatisKDV, AltLimit, Aciklama, GrupId, Durum, SaveDate, SaveUser) values (@Kod, @StokAdi, @Barkodu, @Birimi, @AlisKDV, @SatisKDV, @AltLimit, @Aciklama, @GrupId, @Durum,@SaveDate,@SaveUser)", baglan.bgl());
                                ...
                                else
                                {
                                    ...
                                    komut = new SqlCommand("update Hammaddeler set Kod=@Kod, StokAdi=@StokAdi, Barkodu=@Barkodu, Birimi=@Birimi, AlisKDV=@AlisKDV, SatisKDV=@SatisKDV, AltLimit=@AltLimit, Aciklama=@Aciklama, GrupId=@GrupId, Durum=@Durum, EditDate=@EditDate, EditUser=@EditUser where Id=@Id", baglan.bgl());
                                    komut.Parameters.AddWithValue("@Kod", txtKod.Text);
                                    komut.Parameters.AddWithValue("@StokAdi", txtStokAdi.Text.ToUpper());
                                    komut.Parameters.AddWithValue("@Barkodu", txtBarkodu.Text);
                                    komut.Parameters.AddWithValue("@Birimi", txtBirimi.SelectedItem);
                                ...
                                if (_ac)
                                {
                                    ...
                                    mesajlar.Guncelle(txtStokAdi.Text + " stoğu güncellenmiştir.");
                                    return true;
                                }
                                else
                                {

                                    mesajlar.YeniKayit(txtStokAdi.Text + " stoğu oluşturulmuştur.");
                                    return true;
                                }
        ...
        protected override void SecimYap(object sender)
        {
            ...
            else
            {
                ...
                {
                    formlar.StokGruplarListFormu(true);
