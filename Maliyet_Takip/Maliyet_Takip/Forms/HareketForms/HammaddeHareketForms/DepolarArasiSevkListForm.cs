...
{
    public partial class DepolarArasiSevkListForm : BaseListForm
    {
        ...
        public DepolarArasiSevkListForm()
        {
            InitializeComponent();
        }
        ...
        protected override void Listele()
        {
            ...
            if (AktifKartlariGoster)
            {
                ...
                DataSet ds = komutlar.Dataset("Select * from VW_EVRAKLAR where GCKodu='Sevk' and DonemId='" + AnaForm._donemId + "'", baglan.bgl());
                var lst = ds.Tables[0];
                grid.DataSource = lst;
                baglan.bgl(false);
            }
        ...
        {
            formlar.DepolarArasiSevkEditFormu();
        }
        protected override void Duzelt()
        {
            if (Tablo.FocusedRowHandle > -1)
            {
                ...
                formlar.DepolarArasiSevkEditFormu(SeciliGelecekId, true);
            }
        }
        protected override void EntityDelete()
        {
            try
            {
                if (mesajlar.Sil() == System.Windows.Forms.DialogResult.Yes)
                {
                    ...
                    //db.SubmitChanges();
                    SqlCommand komut = new SqlCommand("Delete from HammaddeHareketleri where EvrakId='" + SeciliGelecekId + "'", baglan.bgl());
