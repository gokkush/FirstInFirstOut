...
{
    public partial class MamulGrupListForm : BaseListForm
    {
        ...
        protected override void Listele()
        {
            ...
            if (AktifKartlariGoster)
            {
                ...
                DataSet ds = komutlar.Dataset("Select * from MamulGruplar where Durum like '" + 1 + "'", baglan.bgl());
                var lst = ds.Tables[0];
                grid.DataSource = lst;
                baglan.bgl(false);
            }
            ...
            {
                grid.DataSource = null;
                DataSet ds = komutlar.Dataset("Select * from MamulGruplar where Durum like'" + 0 + "'", baglan.bgl());
                var lst = ds.Tables[0];
                grid.DataSource = lst;
                baglan.bgl(false);
            }
        ...
        protected override void Duzelt()
        {
            if (Tablo.FocusedRowHandle > -1)
            {
                ...
                formlar.MamulGrupEditFormu(SeciliGelecekId, true);
            }
        }
        protected override void EntityDelete()
        {
            if (mesajlar.Sil() == System.Windows.Forms.DialogResult.Yes)
            {
                ...
                //db.SubmitChanges();
                SqlCommand komut = new SqlCommand("Delete from MamulGruplar where Id='" + SeciliGelecekId + "'", baglan.bgl());
