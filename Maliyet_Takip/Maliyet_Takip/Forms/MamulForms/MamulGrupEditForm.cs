...
{
    public partial class MamulGrupEditForm : BaseEditForm
    {
        ...
        public MamulGrupEditForm(int id, bool ac)
        {
            _ac = ac;
            _id = id;
            InitializeComponent();
            DataLayoutControl = aDataLayoutControl;
            txtKod.Text = numara.MamulGrupNumarasi();
            if (_ac)
            {
                GorevGetir(_id);
            }
        ...
        private void GorevGetir(int id)
        {
            ...
            try
            {
                komut = new SqlCommand("Select * from MamulGruplar where Id = '" + _id + "'", baglan.bgl());
            ...
            if (txtGrupAdi.Text != "")
            {
                ...
                if (dr == System.Windows.Forms.DialogResult.Yes)
                {
                    try
                    {
                        ...
                        {
                            int sayac = 0;
                            komut = new SqlCommand("Select * from MamulGruplar where GrupAdi = '" + txtGrupAdi.Text + "'", baglan.bgl());
                            oku = komut.ExecuteReader();
                            while (oku.Read())
                            {
                                sayac++;
                        ...
                        if (!_ac)
                        {
                            komut = new SqlCommand("insert into MamulGruplar (Kod, GrupAdi, Durum, SaveDate, SaveUser, Aciklama) values (@Kod, @GrupAdi, @Durum, @SaveDate, @SaveUser, @Aciklama)", baglan.bgl());
                        ...
                        else
                        {
                            ...
                            komut = new SqlCommand("update MamulGruplar set Kod=@Kod, GrupAdi=@GrupAdi, Durum=@Durum,EditDate=@EditDate, EditUser=@EditUser, Aciklama=@Aciklama where Id=@Id", baglan.bgl());
                            komut.Parameters.AddWithValue("@Kod", txtKod.Text);
                            komut.Parameters.AddWithValue("@GrupAdi", txtGrupAdi.Text.ToUpper());
                            komut.Parameters.AddWithValue("@Durum", (bool)tglDurum.IsOn);
